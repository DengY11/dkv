...
#ifdef _WIN32
  // Windows doesn't expose a portable directory fsync; best-effort no-op.
  (void)path;
  return Status::OK();
#else
  auto dir = path.parent_path();
  int fd = ::open(dir.c_str(), O_DIRECTORY | O_RDONLY);
  if (fd < 0) return Status::IOError("open parent dir failed: " + dir.string());
  int rc = ::fsync(fd);
  ::close(fd);
  if (rc != 0) return Status::IOError("fsync parent dir failed: " + dir.string());
  return Status::OK();
#endif
}
