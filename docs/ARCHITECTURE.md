...
- **SSTable (`src/sstable.*`)**: immutable sorted runs. Entry encoding: `[deleted byte][seq][klen][vlen][key][value]`. Blocked layout with sparse block index and Bloom filter; footer stores offsets and magic. Optional block cache (`block_cache_capacity_bytes`).
